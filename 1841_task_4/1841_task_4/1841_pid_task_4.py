...
class DroneFly():
	...
	def __init__(self):
		...
		# To obtain info about number of flowers of each color pollinated
		rospy.Subscriber('/red_flower',Int32 , self.set_red_flower)
		rospy.Subscriber('/green_flower',Int32 , self.set_green_flower)
		rospy.Subscriber('/blue_flower',Int32 , self.set_blue_flower)
		...
		self.waypoints=[(7,0.76,23.5),(1.2,7.1,23.5),(1.2,7.1,24.6),(1.2,7.1,21),(-3.7,0.4, 25.2),(-3.7,0.4, 26.3),(-3.7,0.4, 23),(-1,-4.4,23),(-1,-4.4,24.6),(-1,-4,22),(7,0.76,23.5),(7,0.76,32.0)]

		self.wp=0	#Initial waypoint number i.e. first waypoint	
		self.next_waypoint_time=5.0	#interval at which waypoints are changed.
	...
	* Example Call: self.position_hold()
	...
	def position_hold(self):
		...
		while True:
			...
			#if a flower is pollinated before next waypoint timer goes off, move to the net waypoint
			if(self.red_flower.data>self.countr or self.green_flower.data>self.countg or self.blue_flower.data>self.countb):
				if(self.red_flower.data>self.countr):
					self.countr=self.red_flower.data
				if(self.green_flower.data>self.countg):
					self.countg=self.green_flower.data
				if(self.blue_flower.data>self.countb):
					self.countb=self.blue_flower.data
				self.next_waypoint(self.wp)
				self.wp=self.wp+1
				self.last_waypoint_time = self.waypoint_timer
				
			#if the waypoint timer has elapsed, move to next waypoint
	...
	* Example Call: -
	...
	'''	
	* Function Name: set_red_flower()
	* Input: redf
	* Output: -
	* Logic: The subscriber function, which gets the number of flowers of the partiticular colour
	* Example Call: -
	'''	
	def set_red_flower(self,redf):
		self.red_flower=redf
		
	def set_green_flower(self,greenf):
		self.green_flower=greenf
	
	def set_blue_flower(self,bluef):
		self.blue_flower=bluef
	...
	* Example Call: 
	...
	def next_waypoint(self,n):
		#if the current waypoint is the last in the list
		if(n==12):
			self.last_waypoint=True
			print "\nPollination done! Pollinated",self.countr,"Red Daylily"
			print "\nPollination done! Pollinated",self.countg,"Green Carnation"
			print "\nPollination done! Pollinated",self.countb,"Blue Delphinium"
		else:	
			w=self.waypoints[n]
			self.wp_x = w[0]
			self.wp_y = w[1]
